...
Subspace identification methods provide a method to identify LTI systems in their state space form using input-output data. SIMs provide an attractive alternative to prediction error methods because of their ability to identify MIMO systems. Because of their non-iterative solution nature, SIMs are also suitable for identifying systems given large data sets. In general, the subspace identification problem is: given a set of system input and output data, estimate the system matrices ($A$, $B$, $C$, $D$) up to within a similarity transform. 
...
\end{equation}
By Assumption 4, the noise term is uncorrelated with the input sequence. That is,
\begin{equation*}
...
\end{equation}
\end{subequations}
Satisfying condition (\ref{eq:3_instrumental_a}) ensures that we can eliminate the noise term by multiplying Eq. (\ref{eq:3_extended_state_space_noinput}) on the right by $Z_i^T$ and taking the limit for $N\rightarrow\infty$:
...
We follow the general procedure outlined in \cite{trnka2007subspace} to recover the $B$ and $D$ matrices. Recalling the extended state space model is given by
\begin{equation*}
Y_f = \Gamma_f X_k + H_f U_f + G_f E_f
\end{equation*}
...
\end{bmatrix}
\end{equation*}
from which an estimate of the column space of the extended observability matrix can be obtained via an SVD where
\begin{equation}
\hat{\Gamma}_f = US^{1/2}
\end{equation}
